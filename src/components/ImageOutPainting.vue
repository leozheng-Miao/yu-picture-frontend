<template>
  ...
  >
    ...
    <a-flex justify="center" gap="16">
      <a-button type="primary" :loading="isCreatingTask" ghost @click="createTask">生成图片</a-button>
...
// 定义专门用于 控制 “生成图片” 按钮的 loading 的状态
...
const createTask = async () => {
  ...
  try {
    ...
    message.error(`创建扩图任务失败：${error.message || '网络异常'}`)
  } finally {
...
const startPolling = () => {
  ...
  pollingTimer = setInterval(async () => {
    try {
      ...
      if (res.data.code !== 0) {
        message.error(`轮询任务状态失败： ${res.data.message || '请求参数错误'}`)
        clearPolling()
        return
      }
      if (res.data.code === 0 && res.data.data) {
        ...
        if (taskResult.taskStatus === 'SUCCEEDED') {
          ...
          // 弹出详细的报错信息
          //扩图任务执行失败parse input json error: 'image_url'
